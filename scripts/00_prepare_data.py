"""
阶段0：数据准备（HuggingFace 镜像源版本）

背景（重要踩坑，已记入 note.md）：
- AutoDL 国内网络无法访问 storage.googleapis.com（Kaggle 数据实际存储地）
- Kaggle API 能 list 但 download 会卡死在 GCS 重定向
- 解决：改用 HF 镜像 cdtmc/eedi-ir（Eedi 比赛的完整 IR 格式镜像）

cdtmc/eedi-ir 结构：
- corpus: (id_, text) = 2587 条 misconception
- queries: (fold, id_, text) = 4370 条 query（text 含 "SubjectName | ConstructName | Question"）
- qrels: (fold, qid, mid) = query→misconception 金标

本脚本把上述 IR 格式转换为项目统一 schema 并产出：
- misconception_mapping.csv
- long_table.parquet（含 fold）
- folds.parquet
- seen_misc_ids.json
"""
import sys, os
...
def parse_query_text(text: str) -> dict:
    ...
    if q_match:
        ...
        # 前两个 " | " 分隔 Subject | Construct | QuestionText
...
def main():
    ...
    corpus_p = hf_hub_download(IR_REPO, "corpus/train-00000-of-00001.parquet", repo_type="dataset", cache_dir=HF_CACHE)
    queries_p = hf_hub_download(IR_REPO, "queries/train-00000-of-00001.parquet", repo_type="dataset", cache_dir=HF_CACHE)
    qrels_p = hf_hub_download(IR_REPO, "qrels/train-00000-of-00001.parquet", repo_type="dataset", cache_dir=HF_CACHE)
    ...
    misc_name_map = {r["MisconceptionId"]: r["MisconceptionName"] for r in misc_df.iter_rows(named=True)}
    long_df = long_df.with_columns(
        pl.col("MisconceptionId").map_elements(lambda x: misc_name_map.get(x, ""), return_dtype=pl.String).alias("MisconceptionName")
    )
    ...
    # ── 3. 重建 5 折 GroupKFold（by QuestionId）─────
    ...
    subj_dist = long_df.group_by("SubjectName").agg(pl.len().alias("count")).sort("count", descending=True)
    console.print("\n[bold]学科分布（Top10）：")
    ...
    for fold in range(5):
        ...
        console.print(f"  Fold {fold}: train={len(tr)}, val={len(va)}, val中未见错因={unseen_in_val}")

    # ── 5. seen_misc_ids.json（供分数缩放用）────────
    ...
    for s in sample:
        console.print(f"  QID_Ans={s['QuestionId_Answer']}  Subject={s['SubjectName']!r}  Construct={s['ConstructName'][:40]!r}")
        console.print(f"    Q={s['QuestionText'][:60]!r}")
        console.print(f"    Correct={s['CorrectAnswerText'][:30]!r}  Wrong={s['WrongAnswerText'][:30]!r}")
        console.print(f"    → Misconception[{s['MisconceptionId']}]={s['MisconceptionName'][:50]!r}")
