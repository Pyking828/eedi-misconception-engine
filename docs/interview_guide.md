...
**我做了什么：**
1. **召回器（src/eedi/retriever/）**：Qwen3-Embedding-8B + LoRA + InfoNCE / MNRL loss
...
2. **粗排（src/eedi/reranker/pointwise.py）**：Qwen3-Reranker-8B + 打分头
   - cross-encoder：(query, misconception) → 相似度标量
   - top-50 → top-10，减少精排计算量

3. **精排（src/eedi/reranker/listwise.py）**：DeepSeek-R1-Distill-Qwen-14B + LoRA SFT + GRPO
...
3. **CoT 推理 SubAgent（src/eedi/reasoner/）**：DeepSeek-R1-Distill-Qwen-14B
   - 生成"学生为什么会犯这个错误"的推理链
...
**我做了什么：**
1. **SFT**：Qwen3-Embedding-8B（召回）+ Qwen3-Reranker-8B（粗排）+ DeepSeek-R1-Distill-Qwen-14B（精排/推理）
...
### JD 7：大模型 Agent / teacher-student / MCP 协议

**我做了什么：**
1. **本地 32B 离线教师/裁判**：DeepSeek-R1-Distill-Qwen-32B 经 vLLM 批量推理
   - 生成合成 MCQ（覆盖未见错因）
   - 生成 CoT 教师轨迹（蒸馏到3B学生）
...
**如实说明**：受0成本约束，未使用 API 接触商业 100B+ 模型，而是用本地 32B R1 蒸馏模型作为 teacher/judge/蒸馏增强；线上主模型保持 8B/14B，保证可落地。
...
2. **0成本全链路**：无需调用任何付费 API，本地 32B R1 teacher + 14B student + 8B 检索/重排完成蒸馏闭环
