"""
构建 FAISS 索引 + 候选池（sentence-transformers 引擎）。
...
def main():
    ...
    # 保存 misc_ids 顺序（FAISS 行号 → MisconceptionId）
    ...
    console.print(f"[bold]候选池 Recall@{args.top_k}: 全部={ev_all.compute()[f'Recall@{args.top_k}']:.4f} "
