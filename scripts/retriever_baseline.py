"""
Stage 1: retriever zero-shot baseline + LoRA (sentence-transformers).

Usage:
  python scripts/retriever_baseline.py --fold 0 --model Qwen/Qwen3-Embedding-8B
  python scripts/retriever_baseline.py --fold 0 --train --model Qwen/Qwen3-Embedding-8B
  python scripts/retriever_baseline.py --all-folds
"""
...
def evaluate_fold(model, long_df, misc_texts, misc_ids, fold, misc_embs=None, ks=(25, 10)):
    """Evaluate retrieval on the given fold's validation split."""
...
def run_zero_shot(model_name, fold, all_folds=False):
    ...
    for f in folds:
        ...
        console.print(
            f"  [green]Fold {f}: MAP@25={res['MAP@25']}  Recall@25={res['Recall@25']}  Recall@10={res['Recall@10']}  nDCG@25={res['nDCG@25']}"
        )

    if all_folds:
        avg = {
            k: round(np.mean([all_results[f"fold{f}"][k] for f in folds]), 4)
...
def run_lora_train(model_name, fold, epochs, batch_size, lr):
    """LoRA + MultipleNegativesRankingLoss (sentence-transformers Trainer)."""
    ...
    # Build (anchor=query, positive=misconception) pairs
    ...
    model = load_st_model(model_name, cache_dir=HF_CACHE)
    # Short Eedi queries: cap max_seq_length to save memory vs default 32K context.
    ...
    # Max batch with gradient checkpointing (trade compute for VRAM on 8B/14B LoRA).
    ...
    # MNRL = in-batch InfoNCE contrastive loss
    ...
    args = SentenceTransformerTrainingArguments(
        ...
        warmup_ratio=0.05,
        bf16=True,
        batch_sampler=BatchSamplers.NO_DUPLICATES,  # no duplicate positives per batch
        logging_steps=50,
        save_strategy="no",
        report_to=[],
    ...
    # Save LoRA adapter; separate 0.6B baseline vs 8B mainline paths for index scripts.
...
def main():
    ...
    if args.train:
        results["lora"] = run_lora_train(
            args.model, args.fold, args.epochs, args.batch_size, args.lr
        )
    else:
        results["zero_shot"] = run_zero_shot(args.model, args.fold, args.all_folds)

    if args.train:
        out_path = OUTPUT_DIR / f"results_train_fold{args.fold}.json"
    elif args.all_folds:
        out_path = OUTPUT_DIR / "results_baseline_all_folds.json"
    else:
        out_path = OUTPUT_DIR / f"results_baseline_fold{args.fold}.json"
